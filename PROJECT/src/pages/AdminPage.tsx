...
function ProductForm({
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '14px', padding: '28px 32px', marginBottom: '40px' }}>
...
export default function AdminPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#080808', padding: '0 0 80px' }}>
      {/* Top bar */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 40px', borderBottom: '1px solid rgba(255,255,255,0.07)', marginBottom: '40px' }}>
        ...
        <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', letterSpacing: '0.06em' }}>{user.email}</span>
      </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                ...
                {mongoUsers.map(u => (
                  <div key={u._id} style={{ display: 'grid', gridTemplateColumns: '1fr 200px 80px 160px 100px', gap: '16px', alignItems: 'center', padding: '14px 16px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '10px' }}>
                    ...
                    <span style={{ fontSize: '11px', fontWeight: 700, letterSpacing: '0.06em', color: u.role === 'admin' ? '#c6f135' : 'rgba(255,255,255,0.4)', textTransform: 'uppercase' }}>
                      {u.role}
                    </span>
                    ...
                    <div style={{ display: 'flex', gap: '6px' }}>
                      {u._id !== user._id && (
                        <>
                          ...
                          <button onClick={() => handleDeleteUser(u._id)} style={{ padding: '5px 10px', borderRadius: '999px', border: '1px solid rgba(255,80,80,0.3)', background: 'transparent', color: 'rgba(255,100,100,0.8)', fontSize: '10px', cursor: 'pointer' }}>✕</button>
                        </>
                      )}
                    </div>
                  </div>
